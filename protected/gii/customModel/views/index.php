...
$('#CustomModelCode_modelClass').change(function(){
	$(this).data('changed',$(this).val()!='');
});
$('#CustomModelCode_tableName').bind('keyup change', function(){
	var model=$('#CustomModelCode_modelClass');
	var tableName=$(this).val();
	if(tableName.substring(tableName.length-1)!='*') {
		$('.form .row.model-class').show();
	}
	else {
		$('#CustomModelCode_modelClass').val('');
	...
	if(!model.data('changed')) {
		...
		var tablePrefix=$('#CustomModelCode_tablePrefix').val();
...
$('.form .row.model-class').toggle($('#CustomModelCode_tableName').val().substring($('#CustomModelCode_tableName').val().length-1)!='*');
");
?>
<h1>Custom Model Generator</h1>
...
<?php $form=$this->beginWidget('CCodeForm', array('model'=>$model)); ?>
	...
	<div class="row sticky">
		...
		<div class="tooltip">
			This is the prefix that the new base model class to be generated.
			 That the model class to be extended.
		</div>
